...
export const deleteAllNotesFromTrashHandler = function (schema, request) {
  const user = requiresAuth.call(this, request);
  if (!user) {
    return new Response(
      404,
      {},
      { erros: ["The email you entered is not Registered. Not Found Error"] }
    );
  }
  user.trash = [];
  this.db.users.update({ _id: user._id }, user);
  return new Response(200, {}, { trash: [] });
};
